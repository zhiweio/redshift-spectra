...
class TestSettings:
    ...
    def test_sql_security_settings(self, env_vars: dict[str, str]) -> None:
        ...
        with patch.dict(os.environ, extended_vars, clear=False):
            get_settings.cache_clear()
            settings = Settings()
    ...
    def test_is_serverless_true(self, env_vars: dict[str, str]) -> None:
        ...
        serverless_vars = {
            **env_vars,
            "SPECTRA_REDSHIFT_WORKGROUP_NAME": "default-workgroup",
        }
        with patch.dict(os.environ, serverless_vars, clear=False):
            get_settings.cache_clear()
            settings = Settings()
    ...
    def test_auth_mode_options(self, env_vars: dict[str, str]) -> None:
        ...
        for mode in ["api_key", "jwt", "iam"]:
            mode_vars = {**env_vars, "SPECTRA_AUTH_MODE": mode}
            with patch.dict(os.environ, mode_vars, clear=False):
                get_settings.cache_clear()
                settings = Settings()
    ...
    def test_log_level_options(self, env_vars: dict[str, str]) -> None:
        ...
        for level in ["DEBUG", "INFO", "WARNING", "ERROR"]:
            level_vars = {**env_vars, "SPECTRA_LOG_LEVEL": level}
            with patch.dict(os.environ, level_vars, clear=False):
                get_settings.cache_clear()
                settings = Settings()
    ...
    def test_jwt_settings(self, env_vars: dict[str, str]) -> None:
        ...
        with patch.dict(os.environ, jwt_vars, clear=False):
            get_settings.cache_clear()
            settings = Settings()
